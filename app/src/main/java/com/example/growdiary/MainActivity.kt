...
//import com.example.growdiary.firstkid.FirstKidFragment
...
////        val indicatorFirstKid: View = findViewById(R.id.indicator_first_kid)
//
...
////        navIndicators = listOf(indicatorHome, indicatorFirstKid, indicatorDiary, indicatorProfile)
//
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navController.addOnDestinationChangedListener { _, destination, _ ->
            when (destination.id) {
                R.id.diary1, R.id.newDiary, R.id.diary2, R.id.firstKidFragment -> { // TAMBAHKAN firstKidFragment DI SINI
                    bottomNav.visibility = View.GONE
                    fabRoadmap.visibility = View.GONE
                }
                else -> {
